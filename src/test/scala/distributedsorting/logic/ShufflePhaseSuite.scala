...
class ShufflePhaseTest extends FunSuite {
    ...
    val FILES_PER_PARTITION = 2  // 각 (source, partition) 쌍당 파일 개수

    /**
     * 테스트용 파티션 파일 생성
     * 각 워커가 이미 파티션을 완료했다고 가정
     * 각 (source, partition) 쌍에 대해 여러 개의 index 파일 생성
     */
    ...
    test("Shuffle phase: Workers exchange partition files (Local)") {
        ...
        try {
            ...
            // ============================================
            // 6. FileStructure 생성 (다중 index)
            // ============================================
            val fileStructure = createTestFileStructureMultiIndex(NUM_WORKERS, FILES_PER_PARTITION)
            ...
            val expectedFilesPerWorker = NUM_WORKERS * FILES_PER_PARTITION
            
            results.foreach { case (workerId, result) =>
                assertEquals(
                    result.successCount, 
                    expectedFilesPerWorker,
                    s"Worker $workerId should download $expectedFilesPerWorker files"
                )
                ...
                assertEquals(
                    downloadedFiles,
                    expectedFilesPerWorker.toLong,
                    s"Worker $workerId should have $expectedFilesPerWorker files in shuffle_output"
                )
            ...
            println(s"[Test] Total files transferred: ${NUM_WORKERS * expectedFilesPerWorker}")
        ...
        try {
            ...
            results.foreach { case (workerId, result) =>
                assertEquals(
                    result.successCount,
                    expectedFilesPerWorker,
                    s"Worker $workerId should download $expectedFilesPerWorker files via gRPC"
                )
                ...
                assertEquals(
                    downloadedFiles,
                    expectedFilesPerWorker.toLong,
                    s"Worker $workerId should have $expectedFilesPerWorker files in shuffle_output (gRPC)"
                )
